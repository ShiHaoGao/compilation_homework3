...
void Log::output() {
    if (_level == LogLevel::info) {
        llvm::errs() << "\033[32m[Info]: \033[0m" << _message << '\n';
    } else if (_level == LogLevel::warning) {
        llvm::errs() << "\033[35m[Warning]: 033[0m" << _message << '\n';
    } else if (_level == LogLevel::error) {
        llvm::errs() << "\033[31m[Error]: \033[0m" << _message << '\n';
    } else {
        llvm::errs() << "\033[34m[Debug]: \033[0m" << _message << '\n';
    }
}
